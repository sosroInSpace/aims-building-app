...
export async function GET(request: NextRequest) {
    try {
        ...
        let result = await JC_Utils_Business.sqlGet(O_OccupiedModel, code);
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (isList) {
            ...
            const data: O_OccupiedModel = await request.json();
            await O_OccupiedBusiness.Create(data);
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (sortOrderOnly) {
            ...
            await O_OccupiedBusiness.UpdateSortOrder(code, sortOrder);
        } else {
            const data: O_OccupiedModel = await request.json();
            await O_OccupiedBusiness.Update(data);
        }
    ...
    try {
        ...
        if (codes) {
            ...
            for (const singleCode of codeList) {
                await JC_Utils_Business.sqlDelete(O_OccupiedModel, singleCode);
            }
        } else if (code) {
            await JC_Utils_Business.sqlDelete(O_OccupiedModel, code);
        } else {
