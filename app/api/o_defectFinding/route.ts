...
export async function GET(request: NextRequest) {
    try {
        ...
        const result = await JC_Utils_Business.sqlGet(O_DefectFindingModel, code);
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (isList) {
            ...
            const data: O_DefectFindingModel = await request.json();
            await O_DefectFindingBusiness.Create(data);
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (sortOrderOnly) {
            ...
            await O_DefectFindingBusiness.UpdateSortOrder(code, sortOrder);
        } else {
            const data: O_DefectFindingModel = await request.json();
            await O_DefectFindingBusiness.Update(data);
        }
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        await JC_Utils_Business.sqlDelete(O_DefectFindingModel, code);
        return NextResponse.json({ status: 200 });
    } catch (error) {
        console.log(error);
        return NextResponse.json({ error }, { status: 500 });
