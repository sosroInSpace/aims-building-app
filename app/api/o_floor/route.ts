...
export async function GET(request: NextRequest) {
    try {
        ...
        const result = await JC_Utils_Business.sqlGet(O_FloorModel, code);
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (isList) {
            ...
            const data: O_FloorModel = await request.json();
            await O_FloorBusiness.Create(data);
        }
...
export async function POST(request: NextRequest) {
    try {
        const data: O_FloorModel = await request.json();
        await O_FloorBusiness.Update(data);
        return NextResponse.json({ status: 200 });
    } catch (error) {
        console.log(error);
        return NextResponse.json({ error }, { status: 500 });
