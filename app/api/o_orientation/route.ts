...
export async function GET(request: NextRequest) {
    try {
        ...
        let result = await JC_Utils_Business.sqlGet(O_OrientationModel, code);
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (isList) {
            ...
            const data: O_OrientationModel = await request.json();
            await O_OrientationBusiness.Create(data);
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (sortOrderOnly) {
            ...
            await O_OrientationBusiness.UpdateSortOrder(code, sortOrder);
        } else {
            const data: O_OrientationModel = await request.json();
            await O_OrientationBusiness.Update(data);
        }
    ...
    try {
        ...
        if (codes) {
            ...
            for (const singleCode of codeList) {
                await JC_Utils_Business.sqlDelete(O_OrientationModel, singleCode);
            }
        } else if (code) {
            await JC_Utils_Business.sqlDelete(O_OrientationModel, code);
        } else {
