...
export class EmailBusiness_Resend {
    ...
    static async sendContactEmail(emailData: ContactModel) {
        ...
        const htmlContent = await renderComponentToHtml(Template_ContactEmail, emailData);
    ...
    static async sendWelcomeEmail(name: string, email: string) {
        ...
        const { renderComponentToHtml } = await import("@/app/UtilsServer");
        const htmlContent = await renderComponentToHtml(Template_WelcomeEmail, { name, email });
    ...
    static async sendResetPasswordEmail(email: string, theToken: string) {
        ...
        const { renderComponentToHtml } = await import("@/app/UtilsServer");
        const htmlContent = await renderComponentToHtml(Template_ResetPassswordEmail, { token: theToken });
    ...
    static async sendUserVerificationEmail(email: string, theToken: string) {
        ...
        const { renderComponentToHtml } = await import("@/app/UtilsServer");
        const htmlContent = await renderComponentToHtml(Template_UserVerificationEmail, { token: theToken });
    ...
    // Inspection Report
    static async sendInspectionReportEmail(customer: CustomerModel, pdfBase64: string) {
        const resend = new Resend(process.env.RESEND_API_KEY);
        ...
        if (pdfSizeMB < maxAttachmentSizeMB) {
            ...
            const htmlContent = await renderComponentToHtml(Template_InspectionReportEmail, { customer });
            ...
            const pdfUrl = await JC_Utils_Files.uploadPdfAndGetSignedUrl(pdfBase64, s3Key, 7 * 24 * 60 * 59); // 30 days

            const htmlContent = await renderComponentToHtml(Template_InspectionReportEmail, { customer, pdfUrl });
    ...
    static async sendInspectionReportEmailWithLink(customer: CustomerModel, pdfBase64: string) {
        const resend = new Resend(process.env.RESEND_API_KEY);
    ...
    static async sendInspectionReportEmailLinkOnly(customer: CustomerModel) {
        ...
        await resend.emails.send({
            from: `${process.env.NAME} <${process.env.EMAIL_FROM}>`,
            to: "aimsinspectionwebapp@outlook.com",
            subject: `${process.env.NAME} - Building Inspection Report`,
            html: htmlContent
        });
    }
    ...
    static async sendInspectionReportEmailWithSavedFile(customer: CustomerModel, s3Key: string) {
        const resend = new Resend(process.env.RESEND_API_KEY);

        const { renderComponentToHtml } = await import("@/app/UtilsServer");

        // Get signed URL for the existing file (30 days expiry)
        const pdfUrl = await JC_Utils_Files.getSignedUrlForKey(s3Key, 7 * 24 * 60 * 59); // 30 days
    ...
    static async send2FAEmail(email: string, code: string) {
        ...
        const base64MainLogo = fs.readFileSync(path.join(process.cwd(), "public/logos/#Originals/Main [PastelAlt].jpg")).toString("base64");

        const htmlContent = await renderComponentToHtml(Template_TwoFactorAuthEmail, {
            code,
            base64MainLogo
        });
    ...
    static async generateInspectionReportPdf(customer: CustomerModel): Promise<string> {
        const { JC_Utils_Business } = await import("@/app/Utils");
        const { CustomerDefectModel } = await import("@/app/models/CustomerDefect");
        const { DefectImageModel } = await import("@/app/models/DefectImage");
        const Template_InspectionPdf = (await import("@/templates/Template_InspectionPdf")).default;
        ...
        try {
            ...
            const base64TestMainLogo = fs.readFileSync(path.join(process.cwd(), "public/logos/Main [Simple].webp")).toString("base64");
            ...
            while (attempts < maxAttempts) {
                ...
                try {
                    const statusResponse = await axios.default.get(`https://docraptor.com/status/${statusId}`, {
                        headers: {
                    ...
                    if (status === "completed") {
                        // Download the PDF
                        const downloadResponse = await axios.default.get(statusResponse.data.download_url, {
                            responseType: "arraybuffer"
                        });

                        return Buffer.from(downloadResponse.data).toString("base64");
                    } else if (status === "failed") {
                        throw new Error(`DocRaptor PDF generation failed: ${statusResponse.data.validation_errors || "Unknown error"}`);
                    }
                    // Continue polling if status is 'queued' or 'working'
                } catch (statusError) {
                    console.error("Error checking DocRaptor status:", statusError);
            ...
            console.error("Error generating inspection report with DocRaptor:", error);
            throw error;
        }
    }
}
...
export class EmailBusiness_AWS {
    ...
    static async sendInspectionReportEmail(customer: CustomerModel, pdfBase64: string) {
        ...
        if (pdfSizeMB < maxAttachmentSizeMB) {
            ...
            const rawEmail = [`From: ${process.env.NAME} <${process.env.EMAIL_FROM}>`, `To: ${process.env.EMAIL_SUPPORT}`, `Subject: ${process.env.NAME} - Building Inspection Report`, `MIME-Version: 1.0`, `Content-Type: multipart/mixed; boundary="${boundary}"`, ``, `--${boundary}`, `Content-Type: text/html; charset=UTF-8`, `Content-Transfer-Encoding: 7bit`, ``, htmlContent, ``, `--${boundary}`, `Content-Type: application/pdf`, `Content-Transfer-Encoding: base64`, `Content-Disposition: attachment; filename="${filename}"`, ``, pdfBase64, ``, `--${boundary}--`].join("\r\n");
    ...
    static async sendInspectionReportEmailLinkOnly(customer: CustomerModel) {
        ...
        const pdfBase64 = await EmailBusiness_Resend.generateInspectionReportPdf(customer);
    ...
    static async sendInspectionReportEmailWithSavedFile(customer: CustomerModel, s3Key: string) {
        ...
        const command = new SendEmailCommand({
            Source: `${process.env.NAME} <${process.env.EMAIL_FROM}>`,
            Destination: {
                ToAddresses: [customer.Ex_UserEmail || `${process.env.EMAIL_SUPPORT}`]
            },
            Message: {
                Subject: {
                    Data: `${process.env.NAME} - Building Inspection Report`,
