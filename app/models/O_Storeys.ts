...
export class O_StoreysModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Storeys";
    static apiRoute: string = "o_storeys";
    ...
    static async Get(code: string) {
        return await JC_Get<O_StoreysModel>(O_StoreysModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_StoreysModel>(O_StoreysModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_StoreysModel, "");
        }
    }
    static async Create(data: O_StoreysModel) {
        return await JC_Put<O_StoreysModel>(O_StoreysModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_StoreysModel[]) {
        return await JC_PutRaw<O_StoreysModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Storeys");
    }
    static async Update(data: O_StoreysModel) {
        return await JC_Post<O_StoreysModel>(O_StoreysModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_StoreysModel[]) {
        return await JC_PostRaw<O_StoreysModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Storeys");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_StoreysModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Storeys");
    }
    ...
    static jcFieldTypeforField(fieldName: keyof O_StoreysModel) {
        switch (fieldName) {
            case "Code":
