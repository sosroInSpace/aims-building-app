...
export class O_DefectFindingModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_DefectFinding";
    static apiRoute: string = "o_defectFinding";
    ...
    static async Get(code: string) {
        return await JC_Get<O_DefectFindingModel>(O_DefectFindingModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_DefectFindingModel>(O_DefectFindingModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_DefectFindingModel, "");
        }
    }
    static async Create(data: O_DefectFindingModel) {
        return await JC_Put<O_DefectFindingModel>(O_DefectFindingModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_DefectFindingModel[]) {
        return await JC_PutRaw<O_DefectFindingModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_DefectFinding");
    }
    static async Update(data: O_DefectFindingModel) {
        return await JC_Post<O_DefectFindingModel>(O_DefectFindingModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_DefectFindingModel[]) {
        return await JC_PostRaw<O_DefectFindingModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_DefectFinding");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_DefectFindingModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_DefectFinding");
    }
    ...
    constructor(init?: Partial<O_DefectFindingModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.Information = "";
        ...
        this.InformationOverride = undefined;
        Object.assign(this, init);
    }

    static getKeys() {
        return Object.keys(new O_DefectFindingModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_DefectFindingModel) {
        switch (fieldName) {
            case "Code":
