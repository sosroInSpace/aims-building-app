...
export class O_OtherTimberBldgElementsModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_OtherTimberBldgElements";
    ...
    static async Get(code: string) {
        return await JC_Get<O_OtherTimberBldgElementsModel>(O_OtherTimberBldgElementsModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_OtherTimberBldgElementsModel>(O_OtherTimberBldgElementsModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_OtherTimberBldgElementsModel, "");
        }
    }
    static async Create(data: O_OtherTimberBldgElementsModel) {
        return await JC_Put<O_OtherTimberBldgElementsModel>(O_OtherTimberBldgElementsModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_OtherTimberBldgElementsModel[]) {
        return await JC_PutRaw<O_OtherTimberBldgElementsModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_OtherTimberBldgElements");
    }
    static async Update(data: O_OtherTimberBldgElementsModel) {
        return await JC_Post<O_OtherTimberBldgElementsModel>(O_OtherTimberBldgElementsModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_OtherTimberBldgElementsModel[]) {
        return await JC_PostRaw<O_OtherTimberBldgElementsModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_OtherTimberBldgElements");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_OtherTimberBldgElementsModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_OtherTimberBldgElements");
    }
    ...
    static getKeys() {
        return Object.keys(new O_OtherTimberBldgElementsModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_OtherTimberBldgElementsModel) {
        switch (fieldName) {
            case "Code":
