...
export class O_AreasInspectedModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_AreasInspected";
    static apiRoute: string = "o_areasInspected";
    ...
    static async Get(code: string) {
        return await JC_Get<O_AreasInspectedModel>(O_AreasInspectedModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_AreasInspectedModel>(O_AreasInspectedModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_AreasInspectedModel, "");
        }
    }
    static async Create(data: O_AreasInspectedModel) {
        let response = await JC_Put<O_AreasInspectedModel>(O_AreasInspectedModel, this.apiRoute, data);
        return response;
    }
    static async CreateList(dataList: O_AreasInspectedModel[]) {
        return await JC_PutRaw<O_AreasInspectedModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_AreasInspected");
    }
    static async Update(data: O_AreasInspectedModel) {
        return await JC_Post<O_AreasInspectedModel>(O_AreasInspectedModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_AreasInspectedModel[]) {
        return await JC_PostRaw<O_AreasInspectedModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_AreasInspected");
    }
    static async UpdateSortOrder(code: string, sortOrder: number) {
        return await JC_PostRaw(`${this.apiRoute}?sortOrder=true`, { code, sortOrder }, undefined, "O_AreasInspected");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_AreasInspectedModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_Delete(O_AreasInspectedModel, `${this.apiRoute}?codes=${codes.join(",")}`, "");
    }
    ...
    constructor(init?: Partial<O_AreasInspectedModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_AreasInspectedModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_AreasInspectedModel) {
        switch (fieldName) {
            case "Code":
