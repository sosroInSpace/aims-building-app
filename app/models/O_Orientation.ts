...
export class O_OrientationModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Orientation";
    static apiRoute: string = "o_orientation";
    ...
    static async Get(code: string) {
        return await JC_Get<O_OrientationModel>(O_OrientationModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_OrientationModel>(O_OrientationModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_OrientationModel, "");
        }
    }
    static async Create(data: O_OrientationModel) {
        return await JC_Put<O_OrientationModel>(O_OrientationModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_OrientationModel[]) {
        return await JC_PutRaw<O_OrientationModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Orientation");
    }
    static async Update(data: O_OrientationModel) {
        return await JC_Post<O_OrientationModel>(O_OrientationModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_OrientationModel[]) {
        return await JC_PostRaw<O_OrientationModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Orientation");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_OrientationModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Orientation");
    }
    ...
    constructor(init?: Partial<O_OrientationModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_OrientationModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_OrientationModel) {
        switch (fieldName) {
            case "Code":
