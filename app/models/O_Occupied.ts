...
export class O_OccupiedModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Occupied";
    static apiRoute: string = "o_occupied";
    ...
    static async Get(code: string) {
        return await JC_Get<O_OccupiedModel>(O_OccupiedModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_OccupiedModel>(O_OccupiedModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_OccupiedModel, "");
        }
    }
    static async Create(data: O_OccupiedModel) {
        return await JC_Put<O_OccupiedModel>(O_OccupiedModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_OccupiedModel[]) {
        return await JC_PutRaw<O_OccupiedModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Occupied");
    }
    static async Update(data: O_OccupiedModel) {
        return await JC_Post<O_OccupiedModel>(O_OccupiedModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_OccupiedModel[]) {
        return await JC_PostRaw<O_OccupiedModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Occupied");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_OccupiedModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Occupied");
    }
    ...
    constructor(init?: Partial<O_OccupiedModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_OccupiedModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_OccupiedModel) {
        switch (fieldName) {
            case "Code":
