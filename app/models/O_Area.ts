...
export class O_AreaModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Area";
    static apiRoute: string = "o_area";
    ...
    static async Get(code: string) {
        return await JC_Get<O_AreaModel>(O_AreaModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_AreaModel>(O_AreaModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_AreaModel, "");
        }
    }
    static async Create(data: O_AreaModel) {
        return await JC_Put<O_AreaModel>(O_AreaModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_AreaModel[]) {
        return await JC_PutRaw<O_AreaModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Area");
    }
    static async Update(data: O_AreaModel) {
        return await JC_Post<O_AreaModel>(O_AreaModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_AreaModel[]) {
        return await JC_PostRaw<O_AreaModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Area");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_AreaModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Area");
    }
    ...
    constructor(init?: Partial<O_AreaModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_AreaModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_AreaModel) {
        switch (fieldName) {
            case "Code":
