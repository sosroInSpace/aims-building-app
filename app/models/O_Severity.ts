...
export class O_SeverityModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Severity";
    static apiRoute: string = "o_severity";
    ...
    static async Get(code: string) {
        return await JC_Get<O_SeverityModel>(O_SeverityModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_SeverityModel>(O_SeverityModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_SeverityModel, "");
        }
    }
    static async Create(data: O_SeverityModel) {
        return await JC_Put<O_SeverityModel>(O_SeverityModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_SeverityModel[]) {
        return await JC_PutRaw<O_SeverityModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Severity");
    }
    static async Update(data: O_SeverityModel) {
        return await JC_Post<O_SeverityModel>(O_SeverityModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_SeverityModel[]) {
        return await JC_PostRaw<O_SeverityModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Severity");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_SeverityModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Severity");
    }
    ...
    constructor(init?: Partial<O_SeverityModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_SeverityModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_SeverityModel) {
        switch (fieldName) {
            case "Code":
