...
export class O_LocationModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_Location";
    ...
    static async Get(code: string) {
        return await JC_Get<O_LocationModel>(O_LocationModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_LocationModel>(O_LocationModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_LocationModel, "");
        }
    }
    static async Create(data: O_LocationModel) {
        return await JC_Put<O_LocationModel>(O_LocationModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_LocationModel[]) {
        return await JC_PutRaw<O_LocationModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_Location");
    }
    static async Update(data: O_LocationModel) {
        return await JC_Post<O_LocationModel>(O_LocationModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_LocationModel[]) {
        return await JC_PostRaw<O_LocationModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_Location");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_LocationModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_Location");
    }
    ...
    static getKeys() {
        return Object.keys(new O_LocationModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_LocationModel) {
        switch (fieldName) {
            case "Code":
