...
export class O_FurtherInspectionsModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_FurtherInspections";
    static apiRoute: string = "o_furtherInspections";
    ...
    static async Get(code: string) {
        return await JC_Get<O_FurtherInspectionsModel>(O_FurtherInspectionsModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_FurtherInspectionsModel>(O_FurtherInspectionsModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_FurtherInspectionsModel, "");
        }
    }
    static async Create(data: O_FurtherInspectionsModel) {
        let response = await JC_Put<O_FurtherInspectionsModel>(O_FurtherInspectionsModel, this.apiRoute, data);
        return response;
    }
    static async CreateList(dataList: O_FurtherInspectionsModel[]) {
        return await JC_PutRaw<O_FurtherInspectionsModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_FurtherInspections");
    }
    static async Update(data: O_FurtherInspectionsModel) {
        return await JC_Post<O_FurtherInspectionsModel>(O_FurtherInspectionsModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_FurtherInspectionsModel[]) {
        return await JC_PostRaw<O_FurtherInspectionsModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_FurtherInspections");
    }
    static async UpdateSortOrder(code: string, sortOrder: number) {
        return await JC_PostRaw(`${this.apiRoute}?sortOrder=true`, { code, sortOrder }, undefined, "O_FurtherInspections");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_FurtherInspectionsModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_Delete(O_FurtherInspectionsModel, `${this.apiRoute}?codes=${codes.join(",")}`, "");
    }
    ...
    constructor(init?: Partial<O_FurtherInspectionsModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_FurtherInspectionsModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_FurtherInspectionsModel) {
        switch (fieldName) {
            case "Code":
