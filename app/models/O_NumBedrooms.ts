...
export class O_NumBedroomsModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_NumBedrooms";
    ...
    static async Get(code: string) {
        return await JC_Get<O_NumBedroomsModel>(O_NumBedroomsModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_NumBedroomsModel>(O_NumBedroomsModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_NumBedroomsModel, "");
        }
    }
    static async Create(data: O_NumBedroomsModel) {
        return await JC_Put<O_NumBedroomsModel>(O_NumBedroomsModel, this.apiRoute, data);
    }
    static async CreateList(dataList: O_NumBedroomsModel[]) {
        return await JC_PutRaw<O_NumBedroomsModel[]>(`${this.apiRoute}/createList`, dataList, undefined, "O_NumBedrooms");
    }
    static async Update(data: O_NumBedroomsModel) {
        return await JC_Post<O_NumBedroomsModel>(O_NumBedroomsModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_NumBedroomsModel[]) {
        return await JC_PostRaw<O_NumBedroomsModel[]>(`${this.apiRoute}/updateList`, dataList, undefined, "O_NumBedrooms");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_NumBedroomsModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_PostRaw(`${this.apiRoute}/deleteList`, { codes }, undefined, "O_NumBedrooms");
    }
    ...
    static getKeys() {
        return Object.keys(new O_NumBedroomsModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_NumBedroomsModel) {
        switch (fieldName) {
            case "Code":
