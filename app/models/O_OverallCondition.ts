...
export class O_OverallConditionModel extends _Base implements _ModelRequirements {
    static tableName: string = "O_OverallCondition";
    static apiRoute: string = "o_overallCondition";
    ...
    static async Get(code: string) {
        return await JC_Get<O_OverallConditionModel>(O_OverallConditionModel, this.apiRoute, { code });
    }
    ...
    static async GetList(paging?: JC_ListPagingModel, abortSignal?: AbortSignal) {
        if (typeof window !== "undefined") {
            return await JC_GetList<O_OverallConditionModel>(O_OverallConditionModel, `${this.apiRoute}/getList`, paging, {}, abortSignal);
        } else {
            return await JC_Utils_Business.sqlGetList(O_OverallConditionModel, "");
        }
    }
    static async Create(data: O_OverallConditionModel) {
        let response = await JC_Put<O_OverallConditionModel>(O_OverallConditionModel, this.apiRoute, data);
        return response;
    }
    static async CreateList(dataList: O_OverallConditionModel[]) {
        return await JC_PutRaw<O_OverallConditionModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_OverallCondition");
    }
    static async Update(data: O_OverallConditionModel) {
        return await JC_Post<O_OverallConditionModel>(O_OverallConditionModel, this.apiRoute, data);
    }
    static async UpdateList(dataList: O_OverallConditionModel[]) {
        return await JC_PostRaw<O_OverallConditionModel[]>(`${this.apiRoute}?list=true`, dataList, undefined, "O_OverallCondition");
    }
    static async UpdateSortOrder(code: string, sortOrder: number) {
        return await JC_PostRaw(`${this.apiRoute}?sortOrder=true`, { code, sortOrder }, undefined, "O_OverallCondition");
    }
    static async Delete(code: string) {
        return await JC_Delete(O_OverallConditionModel, this.apiRoute, code);
    }
    static async DeleteList(codes: string[]) {
        return await JC_Delete(O_OverallConditionModel, `${this.apiRoute}?codes=${codes.join(",")}`, "");
    }
    ...
    constructor(init?: Partial<O_OverallConditionModel>) {
        super(init);
        this.Code = "";
        this.Name = "";
        this.SortOrder = 999;
    ...
    static getKeys() {
        return Object.keys(new O_OverallConditionModel());
    }

    static jcFieldTypeforField(fieldName: keyof O_OverallConditionModel) {
        switch (fieldName) {
            case "Code":
